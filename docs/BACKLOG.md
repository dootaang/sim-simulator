...
- [ ] 브라우저 종료 후 자동 복구되는 영구 저장소(OPFS/SQLite)
...
- [ ] LLM 구조화 출력의 `memoryCandidates`/`continuityPatch`를 새 계약에 실제 연결
- [ ] hard-negative(비슷하지만 없었던 사건) 주장 검증: 현재 E는 negative abstention 0%, 과잉 abstention 34%
- [ ] 현재 장면 cohort와 지식 보유자 정보를 실제 플레이 세션에서 공급
- [ ] Voyage 실측 후 semantic 단독 후보 하한을 보정; 그 전까지 라이브 기본값은 구조화+어휘(B)

1차 벤치마크 결과: E의 현재 사실 정확도 100%, 폐기값 현재 오노출 0건. 다만 Recall@5 28%로
B(53%)보다 낮고 hard-negative를 거르지 못했다. 따라서 이번 단계는 **안전 계약과 검증 기반의 편입**이며,
실제 플레이 프롬프트 연결은 위 미완료 항목 이후로 보류한다.
