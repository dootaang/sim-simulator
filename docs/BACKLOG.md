...
## 세션 메모 (2026-07-19 — 연애 밸런스 리메이크, 지휘자 직접 구현)

오너 페인포인트("대화는 너무 어렵고 선물은 너무 쉽다 — 선물 너프 없이 대화 쪽에서 해결") 해소:
승급 게이트(수치와 확정 티어 분리 — 대화가 승급 의식이 된다), 대화 수치 2배·실패 위로 +1·
연속 콤보 +1~+3·이야기꽃(선물 직후 같은 시간대 1.5배)·취향 1.5배 곱연산, 세션 보너스 +5.
페이싱 검산: 대화 단독 사랑 확정 4~7일(중앙값 5), 선물만 10일 = 여전히 '첫 만남+승급 대기',
하루 최대 획득 구버전 대비 1.11배(기준 ≤3). 상세는 REPORT-GFL-ROMANCE-BALANCE-2026-07-19.md.

## 세션 메모 (2026-07-19 — 전투 흐름 폴리시, 지휘자 직접 구현)

오너 실측 3대 페인포인트 해소: ①영수증 GFL 매핑(6954065 — 조용한 입금 종결) ②`gfl/sortie/auto`
오토런(quick 전용·정지 5조건·기존 단계 이벤트 재사용 — 같은 시드에서 수동 진행과 최종 상태 해시 동일을
테스트로 증명, LLM 왕복 N→1) ③결정 카드에 전술 3택·개입(게이지 100)·오토런 직접 노출
④"관리 행동 후 관리 화면 유지" 기본값 켬(끔 모드 회귀는 orchestration 2건이 보존). 다음: 연애 밸런스.

## 완료 메모 (2026-07-19 — 7차 연애 리듬)

- [x] 6병과 취향 DC 보정과 발견 전 비공개, 계약 7/30/100일 기념일·당일 1.5배
- [x] 신뢰 이상 외출(시간대 1칸), 전투 트라우마 3회 치유·극복 훈장
- [x] 약속 3종·기한 정산·영수증·동시 3개 상한, 질투 3모드·연속 난입 방지
- [x] 비밀 취미 12종 발견과 전용 캡슐, 사용자 입력 호칭 1회·관계 판정 +1
- [x] 관계 사실을 대화 프롬프트와 장기 기억에 엔진 확정값으로 전달

## 세션 메모 (2026-07-18 밤 — 5차 기지 근무, 지휘자 직접 구현)

오너 결정으로 3차 배치는 지휘자가 직접 구현한다. 1탄 BASE-CREW 완료:

- [x] 시설 근무(병과 적성·기분 경제·수면 절반·훈련/방위/정비/보급 효과), 불만도(문턱·문구 회수,
  규칙 합성), 태만·항명(관계 성공으로 해소)·뜬소문(off/mild/full)·암시장+감사, 가공 라인 2레시피,
  시설 특화 8분기, 소대 회수(404·팀·스쿼드 표기)
- 30일 로테이션 시뮬 실측: 근무 보너스 수입 비중 19.4%(상한 50%), 불만도 최대 0·기분 95 유지
- RNG 규율: end-day 고정 소비 13회(습격 2+사회 11)로 확장 — 기존 습격 테스트 갱신
- 다음: 6차 COMBAT-COMMAND, 7차 ROMANCE-RHYTHM (지휘자 직접)
...
## 세션 메모 (2026-07-18 — 세이브 에폭 도입)

- [x] 업데이트로 스키마 지문이 달라져도 기존 채팅·기억·상태를 보존하고, 과거 사건 원장은 SHA-256으로
  봉인한 뒤 새 에폭에서 플레이를 계속한다.
- [x] 같은 버전의 사건 재생 검증과 세션 무결성 검증은 그대로 유지해 실제 손상은 계속 격리한다.
- [x] 사건 번호는 에폭을 넘어 이어지고, 봉인 경계 이전 undo·분기는 제거한다. 플레이 화면에는 이전
  기록을 봉인했다는 안내가 한 번 표시된다.
- [x] 첫 상태 이주로 GFL의 5칸 제대를 6칸으로 패딩한다. 이주는 멱등이며 불필요한 신규 키를 만들지 않는다.
- [ ] 저장 크기 실측이 필요해지면 봉인 에폭 압축·중복 제거·지연 로드를 별도 설계한다. 30MB 가져오기
  상한은 현재대로 유지한다.

결정 근거와 금지선은 [ADR 0005](adr/0005-save-epochs.md)를 따른다.

## 세션 메모 (2026-07-18 — GFL 관계 티어 정합 핫픽스)

- [x] 선택지 해금 기준을 관계 배열의 절대 인덱스에서 호감도 0 중립 티어 기준 오프셋으로 바꿨다.
  실카드 음수 티어에서는 `차분히 대화한다`만 남고 0/20/50/80/150 문턱에 맞춰 나머지가 열린다.
- [x] 연속 중복 문턱은 첫 항목만 도달 가능하게 해 호감도 400이 `???`가 아니라 `서약`으로 표시된다.
- [x] 기존 talk/nickname/encourage의 id·DC·호감도·기분 수치와 카드의 REL_NAMES·REL_THRES 원본은 유지했다.

## 세션 메모 (2026-07-18 저녁 — 관계 선택지 시스템 개편)

오너 요구("선택지가 3개 고정 — 최초 선택 후 바뀌거나 대화 세션에 진입해야")로 GFL 관계 시스템을 확장했다.

- [x] 관계 선택지 사다리 9종 — 카드 회수 관계 티어(REL_NAMES/REL_THRES)로 해금, 하드코딩 UI 버튼 제거,
  새 셀렉터 `gfl/relation/options`가 잠금 사유·남은 횟수까지 제시(ADR "선택지는 엔진이 제시" 원칙 정합)
- [x] 하루 교류 상한(인형별 4회, 같은 선택지 2회·2회째 효과 절반) — 격려 연타 파밍 차단, 거부는 주사위 전(RNG 소비 0)
- [x] 성공 직후 후속 캡슐 — 엔진이 상태에 기록한 제안만 결정 카드가 되고 DC 보정(-1/크리 -2)도 상태에서만.
  시간대가 넘어가면 소멸
- [x] 티어 승급 시 "특별한 장면 열기" 결정 카드 — 기존 여관 tierCards 패턴을 GFL 판정·대화 이벤트로 확장
- [x] 1:1 대화 세션 (`gfl/relation/session/start`/`end`) — 시계 정지(시간 진행·이동·출격 엔진 거부),
  프롬프트에 집중 블록 주입, 마무리 시 엔진 고정 보너스(호감 +2×난이도·기분 +10, 인형별 하루 1회)
- [x] e2e 1건 신설(관계 캡슐·대화 세션), `저전투력 출격` e2e의 기존 회귀 수정(a7fc56d의 수송 도착
  서사화가 관리창을 닫는데 테스트가 갱신되지 않았던 것 — HEAD에서도 실패하던 건)
- 주의: `gfl/relation/check`의 로그·상태 형태가 확장되어 **기존 GFL 저장 회차는 원장 재생 시 격리 후
  새 회차로 열린다**(기존 복구 경로 그대로, 대화 기록은 보존). 구 선택지 3종의 id·수치는 유지했다.
...
3. ~~진단의 `unsupported_macro ×945` 프롬프트 호환성 문제~~ — 2026-07-18 완료.
   실카드 계수에서 `getvar` 중심임을 확인하고 읽기 전용 조회·비교·조건식만 구현했다. 상태 쓰기 매크로는
   실행하지 않고 이름별 1회 경고로 보존한다. 결과와 분류표는 `REPORT-GFL-MACRO-COMPAT-2026-07-18.md`에 있다.
...
- [ ] **기존 장기 회차 화면 지연 후속 조사** — 같은 Android Chrome에서도 새 채팅은 즉시 반응하고 오래 사용한 회차만 느린 현상을 재현했다. 엔진·브라우저 공통 문제가 아니라 누적 메시지·본문 이미지·응답 분기·구형 저장 잔여의 회차별 비용으로 분류한다. 지금은 수정하지 않으며, 세션 구성별 크기/시간 진단 → 오래된 메시지 가상화·보관 → 무손실 1회 마이그레이션 순으로 별도 발주한다. 원본 백업·엔진 상태·관계·아이템·사건 원장 보존은 합격 관문이다.
...
## 2026-07-18 세션 메모 — GFL 전투 조합

- 원본의 6칸 3열 제대, 임무 세력 문자열, 보스 필드를 회수한다.
- Lucky 합성 전투 규칙으로 병과 역할·열 배치·세력 상성을 결정론 엔진에 연결한다.
- 작전 브리핑과 전투 보고에서 로스터 HP, 상성, 보스를 사용자가 직접 확인할 수 있게 한다.
- 전투 이벤트 형태와 템플릿 지문 변경으로 구 GFL 저장은 새 회차로 격리한다.

## 2026-07-18 세션 메모 — GFL 지휘관 레벨

- 원본 임무 `diff`의 ★ 개수는 별점으로 회수하되, 지휘관 EXP·레벨·효과는 원본에 없는 Lucky 합성 규칙으로 명시한다.
- 승리한 작전만 확정 EXP를 주고 보스·재클리어 계수를 적용한다. 새 주사위는 소비하지 않는다.
- Lv 4/12에서 하루 작전이 4/5회로 늘고, Lv 8/16에서 작전 판정에 +1/+2가 붙는다. 자금·자원·아이템 보상은 추가하지 않는다.
- 전투 개편과 같은 배포 창에 묶어 전투 로그·상태 형태가 바뀐 구 GFL 저장은 새 회차로 한 번만 격리한다.

## 2026-07-18 세션 메모 — GFL 다단계 작전

- 원본 카드의 별점별 3~11단계 표, 임무 유형 3종, 단계 서사 지침 5종, 아이템 drop% 19개를 정적 회수한다.
- 출격 시 전체 단계 시퀀스를 확정해 저장하고, 정찰·돌발·미확인·교전·보스 단계를 엔진 상태로 진행한다.
- 인형 HP와 아이템 전리품은 단계 사이 유지하며 퇴각 시 전리품만 보존한다. 임무 보상과 지휘 EXP는 최종 승리 때 한 번만 지급한다.
- 장비에는 원본 drop 값이 없으므로 작전 루팅에서 제외한다. 단계 가중치와 소모전 보정은 Lucky 합성으로 진단·보고한다.

## 2026-07-18 세션 메모 — GFL 야전 조우

- 원본의 무소속 인형 조우 풀 145명과 ban 목록을 카탈로그 ID로 회수하고, 이미 보유한 인형은 후보에서 제외한다.
- mystery는 루팅 40%·조우 25%·무발견 35%로 나누되 모든 분기에서 사건·대상 RNG 2회를 먼저 소비한다.
- 조우 인형은 숙소 정원을 확인해 무료 즉시 합류한다. 원본에는 별도 비용과 작전당 횟수 상한이 없으며, Lucky는 작전당 1명으로 제한한다.

## 2026-07-18 세션 메모 — GFL 보스 영입·제조 풀

- 실카드의 보스 26명과 전원 능력치 배열을 회수하고, Gebbennu 한 명만 영입 금지로 유지한다. 첫 클리어 기록이 있는 보스만 6성 `BOSS` 병과로 영입할 수 있다.
- `BOSS`는 일반 제대 전투·관계·장비·수복을 공유하지만 MOD와 제조 풀에서는 제외한다. 전투 프로필(어그로 1.5·피해 0.9)은 Lucky 합성이다.
- 장비 제조는 실카드의 일반 22종·중형 29종 풀을 분리 적용하고 시작 시 결과 RNG 한 번만 소비한다. 지시서의 중형 28종 표기보다 실카드가 한 종 많아 회수 우선 원칙에 따라 29종을 보존한다.

## 2026-07-18 세션 메모 — GFL 기록실·습격·해체·상점 대조

- `DOC_DATA` 38건을 원문 그대로 회수하고 완료 작전 2회마다 순서대로 해금한다. 상태에는 완료 누계만 저장하고 해금 여부는 셀렉터가 계산한다.
- 하루 마감 습격은 방위 Lv1 15%~Lv5 3%, 제1제대 자동 방어로 처리한다. 발생 여부와 관계없이 사건·방어 RNG 두 번을 소비한다.
- 원본 인형 분해 보상은 코어 1개·자원 500~2,000이다. 장비 해체에는 원본 환급 규칙이 없어 가격 30% 부품 환급을 합성했다.
- 카리나 상점 19종은 기존 `ITEM_DATA` 19종과 ID가 모두 일치해 누락 추가는 0건이다. 가격 차이는 원본 두 테이블의 대조 결과로 진단에 보존한다.
- 원본 단계 시퀀스 조사에 따라 정찰을 포함한 모든 임무 유형이 battle을 최소 한 번 포함하도록 정정한다.
...
## 2026-07-18 소녀전선 30일 장기 하버스

- [x] 실제 `소녀전선_잔불.png`와 고정 mock LLM으로 30일 루프를 자동 재생한다.
- [x] 10일마다 저장·복원 전체 재생 검증, 같은 시드 재실행의 최종 상태 해시 일치를 확인한다.
- [x] 자유 대화 프롬프트 60개의 전체 곡선과 처음/마지막 10회 평균 비율을 측정한다.
- [x] 자금·자원·HP·숙소 정원·일일 출격 상한 불변식을 매 사건 뒤 검사한다.
- [x] 가짜 이전 스키마 지문으로 에폭 봉인·이주가 한 번만 일어나며 메시지와 기억이 보존되는지 확인한다.

## 2026-07-18 소녀전선 UI 폴리시

- [x] 저격 고용·제대 배치·인형 관리에 검색·병과·등급·정렬·더 보기를 공유하는 인형 선택기를 적용한다.
- [x] 병과 전투 성향에서 권장 열을 계산하고 선택한 인형의 추천 슬롯을 강조한다.
- [x] 관리 화면 아래에 동일한 엔진 결정 카드를 미러링하고, 관리 화면 유지 설정을 저장한다.
- [x] 임무 세력·클리어·보스 표식과 강조된 전투 교전 로그를 표시한다.
- [x] 시설 상세를 한 장씩 펼치고 모바일 헤더를 한 줄 요약으로 접는다.

## 2026-07-19 소녀전선 전투 지휘

- [x] 단계 완주 지휘 게이지와 선언형 개입 3종
- [x] 병과별 MP 자동 스킬과 명중 지원 +5 상한
- [x] 단계당 1회 사기·패닉 및 트라우마 후보 기록
- [x] intel 0/1/2 공개 셀렉터와 포로 심문·매복
- [x] 인형별 격파·치명·방호 전훈과 부대 에이스 사실
