...
async function sqlite3InitModule(moduleArg = {}) {
  ...
  function findWasmBinary() {
    if (Module["locateFile"]) return locateFile2("sqlite3.wasm");
  ...
  Module.runSQLite3PostLoadInit = async function(sqlite3InitScriptInfo, EmscriptenModule, sqlite3IsUnderTest) {
    ...
    globalThis.sqlite3ApiBootstrap = async function sqlite3ApiBootstrap(apiConfig = globalThis.sqlite3ApiConfig || sqlite3ApiBootstrap.defaultConfig) {
      ...
      capi.sqlite3_js_db_export = function(pDb2, schema = 0) {
        ...
        const scope2 = wasm.scopedAllocPush();
        let pOut;
        try {
    ...
    globalThis.WhWasmUtilInstaller = function WhWasmUtilInstaller(target) {
      ...
      target.xWrap = function callee3(fArg, resultType, ...argTypes) {
        ...
        return function(...args) {
          ...
          const scope2 = target.scopedAllocPush();
          try {
            let i = 0;
    ...
    globalThis.Jaccwabyt = function StructBinderFactory(config) {
      ...
      const StructBinderImpl = function StructBinderImpl2(structName, si, opt = /* @__PURE__ */ Object.create(null)) {
        ...
        Object.keys(si.members || {}).forEach((k2) => {
          ...
          makeMemberWrapper.call(self, StructCtor, k2, m);
    ...
    globalThis.sqlite3ApiBootstrap.initializers.push(function(sqlite3) {
          ...
          new wasm.xWrap.FuncPtrAdapter({
            ...
            callProxy: (callback) => {
              return (p, db2, op, zDb, zTbl, iKey1, iKey2) => {
                callback(p, db2, op, wasm.cstrToJs(zDb), wasm.cstrToJs(zTbl), iKey1, iKey2);
              };
            }
          }),
          "*"
      ...
      const __dbCleanupMap = function(pDb2, mode) {
        ...
        if (!mode) {
          ...
          return m;
        } else if (!m && mode > 0) this.dbMap.set(pDb2, m = /* @__PURE__ */ Object.create(null));
        return m;
      ...
      sqlite3.initWorker1API = function() {
        ...
        const wMsgHandler = {
          open: function(ev) {
            ...
            rc.persistent = !!sqlite32.capi.sqlite3_js_db_uses_vfs(db2.pointer, "opfs");
            rc.dbId = getDbId(db2);
          ...
          close: function(ev) {
            ...
            const response = { filename: db2 && db2.filename };
            if (db2) {
          ...
          exec: function(ev) {
            ...
            if (rc.callback || Array.isArray(rc.resultRows)) db2._blobXfer = wState.xfer;
            ...
            try {
              ...
              db2.exec(rc);
              if (void 0 !== changeCount) rc.changeCount = db2.changes(true, 64 === rc.countChanges) - changeCount;
              const lastInsertRowId = !!rc.lastInsertRowId ? sqlite32.capi.sqlite3_last_insert_rowid(db2) : void 0;
    ...
    globalThis.sqlite3ApiBootstrap.initializers.push(function(sqlite3) {
      ...
      opfsUtil.treeList = async function() {
        const doDir = async function callee3(dirHandle, tgt) {
          ...
          tgt.dirs = [];
          tgt.files = [];
          for await (const handle2 of dirHandle.values()) if ("directory" === handle2.kind) {
            const subDir = /* @__PURE__ */ Object.create(null);
            tgt.dirs.push(subDir);
            await callee3(handle2, subDir);
          } else tgt.files.push(handle2.name);
        };
      ...
      opfsUtil.rmfr = async function() {
        ...
        for await (const handle2 of dir.values()) dir.removeEntry(handle2.name, opt);
      };
      ...
      opfsUtil.traverse = async function(opt) {
        ...
        (async function callee3(dirHandle, depth) {
          for await (const handle2 of dirHandle.values()) if (false === opt.callback(handle2, dirHandle, depth)) return false;
...
// src/persistence/sqliteWorker.ts
var scope = globalThis;
var db = null;
var health = { backend: "sqlite-opfs", persistent: false, detail: "not-initialized" };
var queue = Promise.resolve();
function locateFile(name) {
  return new URL(name, import.meta.url).href;
}
function execRows(sql, bind = []) {
  const rows = [];
  db.exec({ sql, bind, rowMode: "object", callback: (row) => rows.push(row) });
  return rows;
}
...
async function initialize() {
  if (db) return health;
  const init = sqlite3_bundler_friendly_default;
  const sqlite3 = await init({ locateFile });
...
function latestValidRevision(id) {
...
function primaryOrRevision(row) {
  if (!row) return null;
  try {
    return rowToSession(row);
  } catch (_) {
    return latestValidRevision(String(row.id));
  }
}
...
scope.onmessage = (event) => {
  ...
  queue = queue.then(async () => {
    try {
      scope.postMessage({ id: message.id, ok: true, result: await handle(message) });
    } catch (error) {
      scope.postMessage({ id: message.id, ok: false, error: error instanceof Error ? error.message : String(error) });
    }
  });
};
