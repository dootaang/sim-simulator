// @ts-nocheck — 업스트림 무수정 이식(ADR 0004). 타입 경계는 façade가 보장한다.
// RisuAI src/ts/process/triggers.ts (kwaroran/RisuAI, GPL-3.0, commit eb7780b) 통이식 — ADR 0004 M-C.
// 트리거 v1(선언형 조건/효과)·v2(triggerCode) 실행기. DB·스토어·UI·LLM·Lua 결합은 TriggerPortEnv 훅으로
// 치환하며, 위험 기능(LLM 호출·이미지 생성·Lua·임베딩)은 기본 무해화 — capability 게이트는 M-D에서 연다.
import { risuChatParser } from '../port/parser.ts';
...
const selectedCharID = miniStore(0);
const getCurrentCharacter = () => __env().getCurrentCharacter();
const getCurrentChat = () => __env().getCurrentChat();
const setCurrentCharacter = (value: any) => __env().setCurrentCharacter(value);
const setDatabase = (value: any) => __env().setDatabase(value);
const getModuleTriggers = () => __env().getModuleTriggers();
const ReloadChatPointer = miniStore(0); const ReloadGUIPointer = miniStore(0);
const parseKeyValue = (value: string) => String(value ?? '').split(String.fromCharCode(10)).map((line) => line.split('=')).filter((pair) => pair.length >= 2 && pair[0].trim()).map((pair) => [pair[0].trim(), pair.slice(1).join('=').trim()] as [string, string]);
const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, Math.min(Number(ms) || 0, 5000)));
const tokenize = async (text: string) => Math.ceil(String(text ?? '').length / 3);
const alertError = (message: unknown) => __env().alert('error', String(message));
...
const requestChatData = async (..._args: any[]) => ({ type: 'fail', result: 'llm_disabled_by_capability' }); // LLM 호출은 M-D capability 게이트에서
const generateAIImage = async (..._args: any[]) => ''; // 이미지 생성 차단(ADR 0004 기본 차단)
const writeInlayImage = async (..._args: any[]) => ''; // 인레이 저장 차단
const runScripted = async (_code: any, arg: any) => ({ stopSending: false, chat: arg?.chat }); // Lua는 M-D(wasmoon capability 게이트)에서 — 그전까지 무해 통과
