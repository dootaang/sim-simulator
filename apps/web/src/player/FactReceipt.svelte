<script lang="ts" module>
  ...
  export function toFactLine(log: Log, index: number): FactLine {
    ...
    if (event.startsWith('gfl/')) {
      ...
      if (event === 'gfl/logistics/collect') { const reward = log.reward as Log | undefined,bonus=[n(log.bonusParts)?`부품 +${n(log.bonusParts)}`:'',n(log.bonusCores)?`코어 +${n(log.bonusCores)}`:''].filter(Boolean).join(' · '); return { key: `${index}`, icon: '📦', label: '군수지원 수령', delta: null, after: null, note: `자금 ${signed(n(reward?.gold) ?? 0)} · 자원 ${signed(n(reward?.res) ?? 0)}${bonus?` · 대성공 d20 ${n(log.bonusRoll)} · ${bonus}`:''}`, rejected: false }; }
      ...
      if (event === 'gfl/relation/check') { const tier = s(log.tier); const tierNote = tier === 'critical_success' ? '대성공' : tier === 'critical_failure' ? '대실패' : tier === 'success' ? '성공' : '실패'; const up = (log.tierChanged as Log | undefined)?.to as Log | undefined; return { key: `${index}`, icon: log.success === true ? '❤️' : '⚠️', label: `${name} · ${s(log.label)}`, delta: deltaStr(n(log.affinityDelta)), after: `호감 ${fmt(n(log.affinity) ?? 0)}`, note: [`d20 ${fmt(n(log.roll) ?? 0)} → ${tierNote}`, (n(log.combo) ?? 0) > 0 ? `교감 ${fmt(n(log.combo) ?? 0)}연속` : '', log.giftGlow === true ? '이야기꽃 ×1.5' : '', up ? `관계 승급 · ${s(up.label)}` : '', !up && s(log.pendingTier) ? `승급 대기 · ${s(log.pendingTier)}` : ''].filter(Boolean).join(' · '), rejected: false }; }
      if (event === 'gfl/relation/session/end') { const up = (log.tierChanged as Log | undefined)?.to as Log | undefined; return { key: `${index}`, icon: '❤️', label: `${name} · 대화 마무리`, delta: deltaStr(n(log.affinityDelta)), after: `호감 ${fmt(n(log.affinity) ?? 0)}`, note: [`기분 ${signed(n(log.moodDelta) ?? 0)}`, up ? `관계 승급 · ${s(up.label)}` : '', !up && s(log.pendingTier) ? `승급 대기 · ${s(log.pendingTier)}` : ''].filter(Boolean).join(' · '), rejected: false }; }
      if (event === 'gfl/relation/outing') { const up = (log.tierChanged as Log | undefined)?.to as Log | undefined; return { key: `${index}`, icon: '❤️', label: `${name} · 외출`, delta: deltaStr(n(log.affinityDelta)), after: null, note: [s(log.place), `기분 ${signed(n(log.moodDelta) ?? 0)}`, up ? `관계 승급 · ${s(up.label)}` : ''].filter(Boolean).join(' · '), rejected: false }; }
