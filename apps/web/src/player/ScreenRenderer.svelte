...
<style>.screen-nav,.actions,.cards{display:flex;gap:var(--space-2);flex-wrap:wrap}.screen-nav{margin-bottom:var(--space-4)}.screen{display:grid;grid-template-columns:minmax(0,1fr) minmax(16rem,var(--sidebar-width));gap:var(--space-4)}.screen>h1{grid-column:1/-1;margin:0}.region{display:flex;flex-direction:column;gap:var(--space-4)}.region-main{grid-column:1}.region-hud,.region-side,.region-actions{grid-column:2}.chat{min-height:18rem}.structured{display:grid;gap:var(--space-2);margin:0}.structured>div{display:grid;grid-template-columns:minmax(6rem,auto) 1fr;gap:var(--space-3);padding:var(--space-2);border-bottom:1px solid var(--color-line)}.structured dt{font-weight:700}.structured dd{margin:0;overflow-wrap:anywhere}@media(max-width:800px){.screen{grid-template-columns:1fr}.region-main,.region-hud,.region-side,.region-actions{grid-column:1}}</style>
