<script lang="ts">
  ...
  let lastLog=$state<Record<string,unknown>[]>([]),quantities=$state<Record<string,number>>({}),wages=$state<Record<string,number>>({}),pending=$state(false);
  ...
  async function run(id:string,params:Record<string,unknown>={},mode:SimulationActionMode='narrated'){if(pending||busy)return;pending=true;try{lastLog=onaction?await onaction({id,params,mode}):runtime.dispatch(id,params).log as Record<string,unknown>[];onchange();}finally{pending=false;}}
  function purchase(){const items=resources.map(value=>({resource:value.id,qty:Math.trunc(quantities[String(value.id)]??0)})).filter(value=>value.qty>0);if(items.length)run('purchase_batch',{items},'ledger');}
...
<div class="console">
  ...
  <section><h2><Icon name="star"/> 영업</h2><div class="actions">{#each waves as wave}<button disabled={busy||pending||!!wave.resolved} onclick={()=>run('traffic_wave',{wave:wave.id})}>{String(wave.label??wave.id)}{wave.resolved?' · 완료':''}</button>{/each}<button disabled={busy||pending} onclick={()=>run('lodging_review')}>숙박 문의</button><button disabled={busy||pending} onclick={()=>run('mail_check')}>우편 확인</button><button disabled={busy||pending} class="danger" onclick={()=>run('day_end')}>하루 마감</button></div></section>
  ...
  <section><h2>재고 구매</h2><div class="purchase">{#each resources as resource}<label><span>{String(resource.label??resource.id)} · 개당 {Number(resource.basePrice??0).toLocaleString()}원</span><input disabled={busy||pending} type="number" min="0" max="999" value={quantities[String(resource.id)]??0} oninput={(event)=>quantities[String(resource.id)]=Number(event.currentTarget.value)}/></label>{/each}<button disabled={busy||pending} onclick={purchase}>설정한 수량 일괄 구매</button></div></section>

  <section><h2>시설</h2>{#each facilities as facility}<article><div><b>{String(facility.label??facility.id)}</b><small>Lv.{String((engineState.facilities as Record<string,unknown>)?.[String(facility.id)]??1)} / {String(facility.maxLevel??1)}</small></div><button disabled={busy||pending} onclick={()=>run('upgrade',{facilityId:facility.id},'ledger')}>증축</button></article>{/each}</section>

  <section><h2>직원</h2>{#each npcs as npc}{@const id=String(npc.id)}{@const hired=staffEntry(id)}<article><div><b>{String(npc.nameKo??npc.nameEn??id)}</b><small>{hired?`고용 중 · 일급 ${Number(hired.dailyWage??0).toLocaleString()}원`:'고용 가능'}</small></div><label class="wage"><input disabled={busy||pending} type="number" min="0" step="1000" value={wages[id]??Number(hired?.dailyWage??10000)} oninput={(event)=>wages[id]=Number(event.currentTarget.value)}/><button disabled={busy||pending} onclick={()=>run(hired?'set_wage':'hire',{npcId:id,dailyWage:wages[id]??Number(hired?.dailyWage??10000)},'ledger')}>{hired?'임금 변경':'고용'}</button></label></article>{/each}</section>
  ...
  <section><h2>의뢰 게시판</h2>{#if quests.length}{#each quests as quest}<article><div><b>{String(quest.name??quest.id)}</b><small>{String(quest.rewardTier??'')}급 보상</small></div><button disabled={busy||pending} onclick={()=>run('attempt_quest',{questId:quest.id})}>수행</button></article>{/each}{:else}<p class="muted">주점 2레벨에서 의뢰 게시판이 열리거나, 현재 가능한 의뢰가 없습니다.</p>{/if}</section>
