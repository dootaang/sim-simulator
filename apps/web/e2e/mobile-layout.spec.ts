...
test('모바일도 데스크톱과 같은 봇 목록·채팅·사이드 패널을 한 화면씩 사용한다',async({page})=>{await page.goto('/');const library=page.getByRole('region',{name:'봇 목록'}),side=page.locator('aside.side');await expect(library).toBeVisible();await expect(page.getByRole('button',{name:'전체 설정'})).toBeVisible();await expect(side).toBeHidden();await page.locator('input[accept=".simpack,.charx,.png,.json"]').setInputFiles({name:'mobile.json',mimeType:'application/json',buffer:card});await expect(page.getByRole('button',{name:'봇 목록'})).toBeVisible();await expect(page.getByText('모바일 채팅')).toBeVisible();await page.getByRole('button',{name:'봇 목록'}).click();await expect(page.getByRole('button',{name:'대화'})).toBeEnabled();await expect(library.getByText('모바일 봇')).toBeVisible();await library.getByRole('button',{name:/모바일 봇/}).click();await page.getByRole('button',{name:'현재 봇 메뉴'}).click();await expect(side).toBeVisible();await expect(side.getByTitle('채팅')).toBeVisible();await expect(side.getByTitle('카드 정보')).toBeVisible();await expect(side.getByTitle('에셋')).toBeVisible();await expect(side.getByTitle('가져오기 진단')).toBeVisible();await page.getByRole('button',{name:'메뉴 닫기'}).click();await expect(side).toBeHidden();await expect(page.getByText('모바일 채팅')).toBeVisible();});
...
test('모바일 설정에서 가져온 Risu 프리셋 토글을 바로 찾고 바꾼다',async({page})=>{
  ...
  await expect(settings.locator('h3')).toContainText('공용 AI 지침');
  await expect(settings.getByText('여러 봇과 대화가 함께 쓰는 지침 보관함입니다.')).toBeVisible();
  ...
  await expect(menu.locator('option[value="toggles"]')).toHaveText('대화 분위기·기능');
  await menu.selectOption('toggles');
  await expect(settings.locator('h3')).toContainText('대화 분위기·기능');
