...
test('기록실 원문을 열람하고 확인 대화상자를 거쳐 인형을 해체한다',async({page})=>{
  ...
  await simulation.getByRole('button',{name:'닫기'}).click(); await expect(simulation).toBeHidden(); await page.getByRole('button',{name:'관리 열기'}).click();
  ...
  await page.getByRole('button',{name:'관리 열기'}).click(); const finalConsole=page.getByRole('dialog',{name:'시뮬레이션'}).getByLabel('소녀전선 지휘 콘솔'); await finalConsole.getByRole('button',{name:'인형',exact:true}).click();
...
test('임무 유형 선택부터 다단계 진행·전투·루팅 영수증까지 완주한다',async({page})=>{
  ...
  await console.locator('.mission-grid').getByRole('button',{name:/ALPHA/}).click();
...
test('군수지원 복귀 보상을 수령하고 심야 작전의 실제 명중 보정을 확인한다',async({page})=>{
  ...
  await console.getByRole('button',{name:/레드·오렌지 작전구역/}).click(); await console.locator('.mission-grid').getByRole('button',{name:/ALPHA/}).click();
...
test('소녀전선 각도괄호 태그를 채팅 본문의 감정 스프라이트로 렌더링한다',async({page})=>{
  ...
  const simulation=await importGfl(page);
  await simulation.getByRole('button',{name:'닫기'}).click();
  await expect(page.getByRole('button',{name:'관리 열기'})).toHaveText('관리 열기');
