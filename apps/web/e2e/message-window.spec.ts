...
test('긴 채팅은 보이는 대화만 그리며 과거를 읽을 때 새 메시지가 위치를 빼앗지 않는다', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    host.style.cssText = 'height:844px;overflow-y:auto';
    ...
    return { total: session.messageCount, initialDomCount, finalDomCount, initialHasLatest, topHasFirst, topBeforeAppend, topAfterAppend };
  });
